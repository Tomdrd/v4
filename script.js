...
// FRASE DO DIA
 // === Lista de frases ===
  ...
  "A felicidade se constrói dia após dia.",
    ...
    // Função que retorna um índice aleatório diferente do anterior
    ...
    // Exibe a frase, escolhendo nova a cada 24h
    function exibirFraseDoDia() {
      const agora = Date.now();
      // Tenta ler e converter o objeto salvo
      let registro = null;
      try {
        registro = JSON.parse(localStorage.getItem(STORAGE_KEY));
      } catch (e) {
        // se JSON inválido, descarta
        registro = null;
      }

      // Se já existe e não passaram 24h, mostra a mesma
      if (registro
      ...
      // Senão, gera nova, salva e mostra
...
${
  ex.imagem
    ? `<img src="${ex.imagem}" style="max-width:100%; border-radius:8px; margin-bottom:10px;">`
    : ""
}
      <div class="linha-inputs">
        <img src="serie.webp" alt="Séries">
        <input type="number" placeholder="Série"  value="${ex.series}" ${
      bloqueado ? "disabled" : ""
    } onchange="atualizarCampo(${index}, 'series', this.value)">
        <img src="repeat.webp" alt="Repetições">
        <input type="number" placeholder="Repetições" value="${
          ex.repeticoes
        }" ${
      bloqueado ? "disabled" : ""
    } onchange="atualizarCampo(${index}, 'repeticoes', this.value)">
        <img src="peso.webp" alt="Carga">
        <input type="text" placeholder="Peso" value="${ex.carga}" ${
      bloqueado ? "disabled" : ""
    } onchange="atualizarCampo(${index}, 'carga', this.value)">
      </div>
      <label><input type="text" class="sem-borda" placeholder="Observação" value="${
        ex.obs
      }" ${
      bloqueado ? "disabled" : ""
    } onchange="atualizarCampo(${index}, 'obs', this.value)"></label>
...
function marcar(index) {
  treinos[diaAtual].exercicios[index].concluido =
    !treinos[diaAtual].exercicios[index].concluido;
  renderizarExercicios();
}
...
function finalizarTreino() {
  ...
  const todosConcluidos =
    treino.exercicios.length > 0 && treino.exercicios.every((e) => e.concluido);
